...
class _ShoppingCartScreenState extends State<ShoppingCartScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          CartBloc(cartController: CartController())..add(ListCartItem()),
      child: Scaffold(
        appBar: AppBar(
          title: Text("My Cart"),
          leading: IconButton(
        ...
        body: BlocBuilder<CartBloc, CartState>(builder: (context, state) {
          ...
          if (state is CartSuccess) {
            final data = (state.data!["data"] as List)
                .map((e) => Cart.fromJson(e))
                .toList();
            final selectedItems = data
                .where((element) => state.selectedItems.contains(element))
                .toList();
            final totalPrice =
                selectedItems.fold(0.0, (double sum, item) => sum + item.price);
            return Column(
              children: [
                ListTile(
                  leading: Checkbox(
                    value: data.every(
                        (element) => state.selectedItems.contains(element)),
                    activeColor: DisColors.primary,
                    onChanged: (value) {
                      BlocProvider.of<CartBloc>(context)
                          .add(SelectAllCartItem(isSelected: value ?? false));
                    },
                  ),
                  title: Text(
                    "Select All",
                ...
                Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: _isNavigatingToCheckout
                          ? null
                          : () async {
